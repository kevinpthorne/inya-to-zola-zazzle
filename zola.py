...
def map_csv_data(input_file, output_file):
    ...
    try:
        ...
        df_output['Name'] = df.get('full_name', None)

        # standardize multiple names
        df_output = df_output[~df_output['Name'].isin(known_dupes)]
        df_output['Name'] = df_output['Name'].str.replace(' and ', ' & ')

        # identify plus one names
        # 1. Create a mask for names that contain ' & ' but don't end with ', & '
        mask = df_output['Name'].str.contains(' & ', na=False) & ~df_output['Name'].str.endswith(', & ')
        # 2. Apply our custom function to the 'Name' column for the selected rows
        #    The result is a new DataFrame with two columns.
        split_results = df_output.loc[mask, 'Name'].apply(split_shared_lastname)
        # 3. Assign the results to the new columns
        df_output[['Full name', 'Plus One']] = split_results
        df_output.loc[(df_output['Plus One'].isnull()) & (df['total_guests_in_party'] > 1), 'Plus One'] = 'Y'

        # No plus one
        # 1. Create a mask to find rows where 'Name' does NOT contain '&'
        mask = ~df_output['Name'].str.contains('&', na=False)
        # 2. Use the mask with .loc to set 'Full name' equal to 'Name' for those rows
        df_output.loc[mask, 'Full name'] = df_output['Name']
        
        # Direct Mappings
        df_output['Email Address'] = df.get('email_address', None).replace(drop_emails, '')
        df_output['Phone Number'] = df.get('phone_number', None)
        df_output['Street Address 1'] = df.get('address_line_1', None).str.title()
        df_output['Street Address 2'] = df.get('address_line_2', None).str.title()
        df_output['City'] = df.get('city', None).str.title()
        df_output['State/Region'] = df.get('state', None)
        df_output['Postal Code'] = df.get('postal_code', None)
        ...
        df_output['Address 1 upper'] = df_output['Street Address 1'].str.upper()
        df_output['City upper'] = df_output['City'].str.upper()
        df_output['Full Name upper'] = df_output['Name'].str.upper()
        ...
        # df_output['dupe'] = df_output.duplicated(subset=['Full Name upper', 'Address 1 upper', 'City upper'], keep=False)
        # df_output = df_output[df_output['dupe']]
        # df_output = df_output.sort_values(by='Street Address 1')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="output_file",
        default="zola.csv",
        help="The path for the output file. Defaults to 'zola.csv' in the current directory."
    )
